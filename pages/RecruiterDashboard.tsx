...
const RecruiterDashboard: React.FC<RecruiterDashboardProps> = ({ user, candidates, isInitialLoading = false, onAdminAction, onUpdateCandidate, onRefreshCandidates, onLogout }) => {
  ...
  const [userDeleteTarget, setUserDeleteTarget] = useState<DashboardUserEntry | null>(null);
  ...
  const loadInquiries = useCallback(async (options?: { foreground?: boolean }) => {
    ...
    const showForegroundLoading =
      options?.foreground === true && !inquiriesLoadedOnce.current && inquiriesCount.current === 0;
    if (showForegroundLoading) setIsLoadingInquiries(true);
    try {
  ...
  const dashboardUsers = useMemo<DashboardUserEntry[]>(() => {
    const merged = new Map<string, DashboardUserEntry>();
    for (const userItem of registeredUsers) {
  ...
  const filteredRegisteredUsers = useMemo(() => {
    ...
    return dashboardUsers.filter(
      (u) =>
        ...
        (u.sourceCandidateNumber || '').toLowerCase().includes(term) ||
                ...
                <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                  ...
                  <div className="flex shrink-0 flex-wrap items-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => void loadInquiries({ foreground: inquiries.length === 0 })}
                    >
                      Aktualisieren
                    </Button>
                  </div>
            ...
            <div className="overflow-hidden rounded-2xl border border-orange-200/40 bg-[#101B31] shadow-sm">
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {filteredRegisteredUsers.length !== dashboardUsers.length && (
              ...
              {loadingRegisteredUsers && dashboardUsers.length === 0 ? (
                ...
                <div className="px-4 py-8 text-center text-xs font-medium text-slate-300">
                  {dashboardUsers.length === 0 ? 'Keine Nutzer gefunden.' : 'Keine Treffer für die Suche.'}
                </div>
              ) : (
                <>
                      ...
                      return (
                        ...
                        >
                          ...
                          <Button
                            ...
                            disabled={!canDeleteUser || deletingUserId === u.id}
                            title={
                              u.isExternallyAdded
